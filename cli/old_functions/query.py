...
def searchWarehouseHowMany(stocklist, warehouse_number, item_to_search):
    ...
    for x in stocklist:
        if x["warehouse"] == warehouse_number:
            ...
            if (
                full_item_name.lower() == item_to_search.lower()
...
def availabilityAnditemDaysInStock(stocklist, item_to_search, warehouses):
    ...
    for warehouse in warehouses:
        ...
        counts["warehouse " + str(warehouse)] = searchWarehouseHowMany(
            stocklist, int(warehouse), item_to_search
        )
...
while repeat:
    ...
    if action == 1:
        ...
        for warehouse in warehouses:
            ...
            input(
                "Press any key to continue to next warehouse",
            )
