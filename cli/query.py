...
def availabilityAnditemDaysInStock(stocklist, item_to_search):
    ...
    if sum(counts.values()) != 0:
        ...
        for warehouse in stocklist:
            for item in warehouse.stock:
                if (
                    item.__str__().lower() == item_to_search.lower()
                ):  # the .lower makes search case insensitive
...
def printCategories(stocklist):
    ...
    for warehouse in stocklist:
        for x in warehouse.stock:
            categoryDictionary.setdefault(
                x.category, 0
            )  # imagine this as an empty vase with 0 items inside
            categoryDictionary[
                x.category
            ] += 1  # increasing the count of the items in this category(vase), instead of creating a new category

    count = 1  # just prints 1.2.3...
    categoryList = (
        []
    )  # preparing an empty lists to add the categories in an ordered way
    ...
    try:
        ...
        print(
            "List of", categoryList[category_to_browse - 1], "available:"
        )  # in reality the nth item is the -1 index,
        ...
        print("-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-")
        addAction(
            "Printed Categories for " + categoryList[category_to_browse - 1] + " Items"
        )
    except:
...
def findUser(personnel, user_name):
    for person in personnel:
        if person.is_named(user_name):
            return person  # If we found the user, return as we don't have to search any more

    # As a last action we have to see if someone is an subordinate of another person
    # according to the instructions, these persons should also be able to order.
    for person in personnel:
        if person.head_of:
            for subordinate in person.head_of:
                subordinate.setdefault(
                    "head_of", []
                )  # setting a default head_of on the data.py list of dictionaries so that we can use it for creating a new Employee object
                subUser = Employee(
                    subordinate["user_name"],
                    subordinate["password"],
                    subordinate["head_of"],
                )  # Creating a new Employee object, based on the items inside the current list
                foundUser = findUser(
                    [subUser], user_name
                )  # searching recursively on the new Object to find a person that we could be heads off
                if (
                    foundUser != None
                ):  # if we didn't find anything we should return None
...
# Need to add this here, otherwise the program will run at import of tests
    ...
    while repeat:
        ...
        if action == 1:
            ...
            if user.is_authenticated == False:
                password = input(
                    "Please input your password in order to do this action: "
                )
                user.authenticate(password)

            while user.is_authenticated == False:
                print(
                    "You are not allowed to perform this action until you are Authenticated as an Employee."
                )
