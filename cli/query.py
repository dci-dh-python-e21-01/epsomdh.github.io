...
#checking if we have enough items
           if item_number <= (elm_count_1 + elm_count_2):#if yes
             print("The order has been placed",item_number)#make order
           else:#if not
            print("There are not this many items available.The maximum amount that can be ordered is ",(elm_count_1 + elm_count_2))
            max_order=int(input("Would you like to order the maximum amount available?,\n1.Yes\n2.No\n:---Type the number of the operation:  "))
            if max_order==1:
              print("An order of",(elm_count_1 + elm_count_2),item_name,"has been placed")
