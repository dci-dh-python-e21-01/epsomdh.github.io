...
def findUser(personnel, user_name):
    for person in personnel:
        if person.is_named(user_name):
            return (
                person  # If we found the user, return as we dont have to seach any more
            )
    ...
    for person in personnel:
        if person.head_of:
            for subordinate in person.head_of:
                ...
                foundUser = findUser(
                    [subUser], user_name
                )  # searching recursively on the new Object to find a perosn that we could be heads off
        ...
        if user.is_authenticated:
            ...
            if order_action == 1:
                ...
                if item_number <= sum(counts.values()):  # if yes
                    print(
                        "An order of", item_number, item_to_search, "has been placed"
                    )  # make order
                    addAction(
                        "Made an order for "
                        + str(item_number)
